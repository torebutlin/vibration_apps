...
export function initUI(state, engine, callbacks) {
  ...
  function buildHelp() {
    ...
    addSection('Plot', [
      ['Zoom', 'Drag across the spectrum to zoom the frequency axis, or pinch it — on the spectrogram, pinch up the frequency axis (scroll wheel on both). Double-click or double-tap comes back to the Range setting.'],
      ...
      ['Averaging', 'Both modes average the power spectrum, and both update every 21 ms. Exp weights the past exponentially: recent data counts for most, old data fades but never quite leaves. Linear weights the last N independent spectra equally and drops them as they age — a moving Welch estimate. The time beside Averages is how far back that window reaches, N half-windows of data (N·N_fft/2fs); Clear empties it at once. Off shows single periodograms, which is what the grey live trace always is.'],
